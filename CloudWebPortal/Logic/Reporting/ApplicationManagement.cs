...
namespace CloudWebPortal.Logic.Reporting
{
    ...
    public class ApplicationManagement
    {
        ...
        private Aneka.Entity.ServiceAddress serviceAddress;

        public ApplicationManagement(String username, String password, String masterHostIP, int masterPortNumber)
        {
            credentials = new UserCredentials(username, password);
            serviceAddress = new Aneka.Entity.ServiceAddress(ContainersManagement.GetContainerUri(masterHostIP, masterPortNumber), "AccountingService");
        }
        ...
        public List<IAccountable> getDoneApplications(appQureyType.Type data, DateTime? fromDate, DateTime? toDate)
        {
            ...
            try
            {
                ...
                Aneka.Accounting.AccountingManager manager = new AccountingManager(serviceAddress);
                ...
                appViewList = manager.Query(credentials, typeof(Aneka.Entity.ApplicationView), request);

                if (data == appQureyType.Type.NonSuccessfulOnly)
                {
                    foreach (Aneka.Entity.ApplicationView app in appViewList)
                    {
        ...
        public void AbortApplication(String ApplicationMasterService, String ApplicationID)
        {
            try
            {
                Aneka.Entity.ApplicationAbortMessage abortMsg = new Aneka.Entity.ApplicationAbortMessage("AccountingManager", ApplicationMasterService, ApplicationID);
